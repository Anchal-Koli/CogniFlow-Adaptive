...
const Quizzes = () => {
  ...
  const topics = useMemo(() => {
    const uniqueTopics = Array.from(
      new Set(quizzes.map((q: any) => q.topic).filter(Boolean))
    );
  ...
  const filteredQuizzes = useMemo(() => {
    return quizzes.filter((quiz: any) => {
      ...
      const matchesDifficulty =
        difficulty === "all"
          ? true
          : quiz.difficulty.toLowerCase() === difficulty;
  ...
  }, [quizzes, search, difficulty, topic, courseId]);
...
>
  ...
  <h3 className="text-xl font-bold text-foreground mb-2">
    Quiz Collection Coming Soon
  </h3>

  <p className="text-sm text-muted-foreground max-w-lg mx-auto leading-7">
    This course is currently available for conceptual learning and guided study.
    Adaptive quizzes for this learning path are still being prepared and will
    be added soon.
  </p>
  ...
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
    ...
    <div className="rounded-xl border border-border bg-secondary/20 p-4">
      ...
      <p className="text-xs text-muted-foreground">
        Continue exploring course material and strengthen fundamentals.
      </p>
    </div>
    ...
    <div className="rounded-xl border border-border bg-secondary/20 p-4">
      ...
      <p className="text-xs text-muted-foreground">
        Personalized quizzes will appear automatically based on mastery.
      </p>
    </div>
    ...
    <div className="rounded-xl border border-border bg-secondary/20 p-4">
      ...
      <p className="text-xs text-muted-foreground">
        Your future quiz performance will update topic mastery dynamically.
      </p>
    </div>
  ...
  <button
    ...
    className="mt-8 px-5 py-3 rounded-xl bg-primary text-primary-foreground font-semibold hover:opacity-90 transition-all"
  >
          ...
          {filteredQuizzes.map((quiz: any, index: number) => (
            ...
            >
              ...
              <p className="text-sm text-muted-foreground mt-2 line-clamp-2 min-h-[40px]">
                {quiz.description ||
                  "Practice this quiz to strengthen your understanding."}
              </p>
              ...
              <div className="grid grid-cols-3 gap-3 mt-5">
                ...
                <div className="rounded-xl border border-border bg-secondary/20 p-3 text-center">
                  <Trophy
                    size={14}
                    className="text-mastery mx-auto mb-1"
                  />
